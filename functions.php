...
function my_slide_script_init() {
    wp_enqueue_style( 'swiper', get_template_directory_uri() . '/css/swiper-bundle.min.css', array( ), '7.4.1', 'all' );
    wp_enqueue_script( 'swiper', get_template_directory_uri() . '/js/swiper-bundle.min.js', array(), '7.4.1', true );
    wp_enqueue_script( 'script.js', get_template_directory_uri() . '/js/script.js', array(), '7.4.1', true );
}

add_action( 'wp_enqueue_scripts', 'my_slide_script_init' );
...
function my_the_post_category($anchor = true,$id = 0){
    ...
    $cat_id = $this_categories[0]->term_id;
    $cat_name = $this_categories[0]->cat_name;
    if($this_categories[0]){
        if($anchor){//引数がtrueならリンク付きで出力
            echo '<a href="'.esc_url(get_category_link($cat_id)).'">'.esc_html($cat_name).'</a>';
